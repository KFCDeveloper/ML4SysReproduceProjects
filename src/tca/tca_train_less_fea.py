...
def testing(epoch, nn_model, log_file):
    ...
    # run test script
    os.system('python tca_test_less_fea.py --nn_model ' + nn_model
...
def choose_and_combine(train_dataset_one, train_dataset_two):
    portion = 0.0001
    ...
    tca_tool = tca_rel.TCA(kernel_type='linear', dim=5, lamb=1, gamma=1)
    new_train_one, new_train_two = tca_tool.fit(subset_train_one_X, subset_train_two_X)
...
if __name__ == '__main__':
    ...
    dataset0_cooked_time, dataset0_cooked_bw, _ = load_trace.load_trace(args.src)
    dataset1_cooked_time, dataset1_cooked_bw, _ = load_trace.load_trace(args.trg_train)

    # 有了src dataset 和 target dataset 去合成 dataset
    combined = choose_and_combine(args.src,args.trg_train)
    # save combined, 然后combined 保存到
