...
# 这下面的6个层，要对应到那6个features，不能随便删，env里面的feature注释掉第几个，这边也注释掉对应的
class Actor(nn.Module):
    ...
    def forward(self, inputs):
        ...
        split_4 = F.relu(self.conv3_actor(inputs[:, 2:3, :self.a_dim]).view(-1, FEATURE_NUM))
        split_5 = F.relu(self.fc3_actor(inputs[:, 3:4, -1]))

        merge_net = torch.cat([split_0, split_1, split_4, split_5], 1)
