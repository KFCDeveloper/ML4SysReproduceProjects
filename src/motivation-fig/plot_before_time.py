...
def qoe_cdf(outputs):
    ...
    points = {'car':'-o','ferry':'-^', 'bus':'-*', 'bus_global':'-h'}
    colors = {'car':'#000000','ferry':'#d62728', 'bus':'#1f77b4', 'bus_global':'gray'}
    names_list = {'car':'car','ferry':'ferry', 'bus':'bus', 'bus_global':'bus_global'}
    legend_position = {'car':'lower center','ferry':'upper left', 'bus':'lower left'}
    trace_index = {'car':4,'ferry':10, 'bus':5, 'bus_global':5}
# 1 4 4
# 1 5 4
# 124
# 1 2 11
...
# 4 2 18
# 4 6 18 
    app_dir_name = '/mydata/Pensieve-PPO/src/motivation-fig/app-three-together-bustest/'
    for each_LOG in os.listdir(app_dir_name):
        ...
        for idx, scheme in enumerate(SCHEMES):
            ...
            for scheme in reward_all:
                ...
                ax.plot(rewards, points[app_name], lw=3.0 ,color=colors[app_name],label='%s: %.2f' % (names_list[app_name], np.mean(rewards)))  # 直接使用 "实验" 作为标签
                points_index += 1
