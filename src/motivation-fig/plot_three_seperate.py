...
def qoe_cdf(outputs):
    ...
    for each_LOG in os.listdir(app_dir_name):
        ...
        for idx, scheme in enumerate(SCHEMES):
            ...
            for scheme in reward_all:
                ...
                ax.plot(rewards, points[points_index], lw=3.0 ,color=colors[points_index],linestyle=line_styles[points_index],label='%s: %.2f' % (names_list[points_index], np.mean(rewards)))  # 直接使用 "实验" 作为标签
                points_index += 1
    ...
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    ax.legend(fontsize=17, ncol=1, edgecolor='white',loc=legend_position[app_name])
