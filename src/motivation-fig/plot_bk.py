...
def qoe_cdf(outputs):
    ...
    max_bitrate = 0
    for each_LOG in os.listdir("/mydata/Pensieve-PPO/src/motivation-fig/app-car/"):
        each_complete_LOG = "/mydata/Pensieve-PPO/src/motivation-fig/app-car/" + each_LOG
        for idx, scheme in enumerate(SCHEMES):
            mean_arr = []
            ...
            ax.plot(base[:-1], cumulative, '-', lw=LW, \
                    label='%s: %.2f' % (each_LOG, np.mean(mean_arr)))
...
if __name__ == '__main__':
    ...
    qoe_cdf('/mydata/Pensieve-PPO/src/motivation-fig/baselines-qoe-car')