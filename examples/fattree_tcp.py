...
n_classes_per_port = 4  # ydy: seem like num of priority
...
for n in ft.nodes():
    node = ft.nodes[n]
    for port_number, next_hop in node["port_to_nexthop"].items():   # ft.nodes[next_hop]["device"] 全是switch,没有generator
        node["device"].ports[port_number].out = ft.nodes[next_hop]["device"]    # node["device"].ports[port_number] is class Port; demux.outs is [class Port]
