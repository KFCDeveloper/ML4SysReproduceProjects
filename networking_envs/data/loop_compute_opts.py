...
topo_full_name = sys.argv[1]   # Abilene-squeeze-links-more1
    ...
    for i in range(len(opts_info)):
        ...
        for j in range(opts_info[i][1]):
            ...
            with(open(opts_dir_prefix + d + '/' + input_file_name)) as f:
                ...
                for line in lines:
                    if 'Optimal result for actual demand:' in line:  # if line.startswith(' Optimal result for actual demand: '):
                        opt_res = float(line[line.rfind(': ')+1:])  # to find the last ':'
