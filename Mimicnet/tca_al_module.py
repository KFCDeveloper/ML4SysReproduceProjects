...
#             )
#             interval_forboth = interval_latency + interval_drop
#             uncertainties.append(interval_forboth.cpu().numpy())
#             index += batch_size
...
#                               batch_size):
#     # Print the dimensions and values of inputs
#     print("large_train_samples shape:", np.array(large_train_samples).shape)
#     print("large_train_d_targets shape:", np.array(large_train_d_targets).shape)
#     print("large_train_l_targets shape:", np.array(large_train_l_targets).shape)
...
#     # Create train_batch
#     train_batch = [
#         large_train_samples[index + i: index + i + window_size]
#         for i in range(batch_size)
#     ]
#     train_batch = np.array(train_batch)
#     print("train_batch shape after conversion:", train_batch.shape)

#     # Generate d_label_batch
#     d_label_batch = [
#         large_train_d_targets[index + i + window_size - 1]
#         for i in range(batch_size)
#     ]
#     d_label_batch = np.array(d_label_batch).reshape(len(d_label_batch), 1)
#     print("d_label_batch shape:", d_label_batch.shape)
#     print("d_label_batch values:", d_label_batch)

#     # Generate l_label_batch
#     l_label_batch = [
#         large_train_l_targets[index + i + window_size - 1]
#         for i in range(batch_size)
#     ]
#     l_label_batch = np.array(l_label_batch).reshape(len(l_label_batch), 1)
#     print("l_label_batch shape:", l_label_batch.shape)
#     print("l_label_batch values:", l_label_batch)
...
def AL_intrain(sampled_uncertainties, budget, sampled_costs,top_k):
    # sampled_uncertainties_flat = list(chain.from_iterable(sampled_uncertainties))
    sampled_uncertainties_flat = [u.item() for u in chain.from_iterable(sampled_uncertainties)]
    ...
    top_k_uncertainties = sorted_uncertainties[:int(top_k*len(sorted_uncertainties))]
    ...
    # print('number of data select in sample {}'.format(i))
    # Return the sum of the selected uncertainties
    return top_k_uncertainties

# def uncertainties_simulation(tr_dataloader,dataset_size,sample_size,iter_training_cost,pool_indices,model,opt,device):
def uncertainties_simulation(X_select, y_d_select, y_l_select,dataset_size,sample_size,iter_training_cost,pool_indices,model,enable_validation,index,batch_size,window_size,lengthcurrent,random_indices_simulation,AL_select,AL_leftover):
    # sample_indice = random.sample(pool_indices, sample_size)
    # subset = Subset(tr_dataloader, sample_indice)
    # sample_dataloader = DataLoader(subset, batch_size=tr_dataloader.batch_size, shuffle=False)
    ...
    sampled_uncertainties = uncertainty(model,X_sample,y_d_sample,y_l_sample,enable_validation,index,batch_size,window_size)
    print(f"sampled_uncertainties: {len(sampled_uncertainties)}")
...
def calculate_and_compare_metrics(uncertainties, last_uncertainty, current_uncertainty, dataset_size, iter_training_cost, data_uncertainty, prev_smoothed_delta_cost_ratio, prev_smoothed_diff_sum_cost_ratio, smooth_delta_cost_ratios, smooth_diff_sum_cost_ratios, window_size, epoch,AL_select,AL_leftover,selection_cost):
    """
    Calculate various metrics and perform moving average comparison.
    ...
    mean_simulation = sum(data_uncertainty) / len(data_uncertainty)
    
    print(f"mean_simulation: {mean_simulation}")
    print(f"len(data_uncertainty): {len(data_uncertainty)}")
    
    diff = delta + (mean_simulation - mean_uncertainty) * AL_select
    print("delta:", delta)
    print("mean_simulation:", mean_simulation)
    print("mean_uncertainty:", mean_uncertainty)
    print("AL_select:", AL_select)
    
    size_increase = (dataset_size + len(data_uncertainty)) / dataset_size
    sum_cost = iter_training_cost * size_increase + selection_cost
    delta_cost_ratio = delta / iter_training_cost
    diff_sum_cost_ratio = diff / sum_cost
...
def active_learning_iteration(cost_utility, i, model, X, y_d, y_l, pool_indices,chunk_indices,pool_trunk_indices):
    ...
    # y_l_pool = y_l[pool_indices]
        ...
        # costs = df.loc[valid_indices, 'time_cost'].tolist()
        valid_indices = [idx for idx in range(start_idx, end_idx) if idx < len(df)]
        costs = [cost for cost in df.loc[valid_indices, 'time_cost']]
    ...
    if i == 0:
        ...
        if len(chunk_representations) <= number_cluster:
            print("Number of chunk representations is less than or equal to the number of clusters.", flush=True)
            
            # Sort the chunks by their utility in descending order
            sorted_chunks = sorted(range(len(chunk_representations)), key=lambda x: trunk_uncertainty_cost_sums[x][1], reverse=True)
            ...
            for idx in sorted_chunks:
                # Check if adding this chunk exceeds the cost utility
                if cumulative_utility + chunk_cost[idx][1] > cost_utility:
                    print(f"Skipping chunk {chunk_cost[idx][0]} due to cost constraint.", flush=True)
                    continue
                
                # Add this chunk to the selection
                cumulative_utility += chunk_cost[idx][1]
                iteration += 1
                print(f"iteration: {iteration}, idx: {idx}, cost: {chunk_cost[idx][1]}, cumulative_utility: {cumulative_utility}", flush=True)
                actual_selected_chunks.append(chunk_cost[idx][0])
    
            print("Completed selection for fewer chunks than clusters.", flush=True)
            print(f"Final cumulative utility: {cumulative_utility}", flush=True)
            print(f"Number of selected chunks: {len(actual_selected_chunks)}", flush=True)
            
            return num_to_select, actual_selected_chunks, cumulative_utility
...
def active_learning_iteration_no_clustering(cost_utility, i, model, X, y_d, y_l, pool_indices,chunk_indices,pool_trunk_indices):
    print(f"Starting active learning iteration {i}", flush=True)
    # pool_loader = DataLoader(Subset(dataloader.dataset, pool_indices), batch_size=batch_size, shuffle=False)
    print(f"pool_trunk_indices: {pool_trunk_indices}", flush=True)
    trunk_uncertainty_cost_sums = []
    chunk_rep_with_indices = []
    chunk_cost = []
    for trunk_idx in pool_trunk_indices:
    ...
    sum_utility = []
    for _, cost in trunk_uncertainty_cost_sums:
        sum_utility.append(cost)
    
    probabilities = [sum_utility[i] / sum(sum_utility) for i in range(len(sum_utility))]
    ...
    if i == 0:
        ...
        actual_selected_chunks = []
        iteration_count = 0
        while len(actual_selected_chunks) < num_to_select:
        # Randomly select a position based on probabilities
            selected_pos = random.choices(range(len(pool_trunk_indices)), weights=probabilities, k=1)[0]
            selected_idx = pool_trunk_indices[selected_pos]
            print(f"selected_idx: {selected_idx}", flush=True)
            # Check if the trunk is already selected
            if selected_idx not in actual_selected_chunks:
                actual_selected_chunks.append(selected_idx)
                # Retrieve the cost of the selected trunk
                selected_cost = next(cost for idx, cost in chunk_cost if idx == selected_idx)
                cost_utility += selected_cost  # Add the cost to the total
                print(f"Selected trunk: {selected_idx}, Cost: {selected_cost}, Total Cost: {cost_utility}", flush=True)
        ...
        trunk_indices, chunk_representations = zip(*chunk_rep_with_indices)
        print("Starting K-means clustering...", flush=True)
        current_cost_utility = 0
        actual_selected_chunks = []
        while current_cost_utility < cost_utility:
        # Randomly select a position based on probabilities
            selected_pos = random.choices(range(len(pool_trunk_indices)), weights=probabilities, k=1)[0]
            selected_idx = pool_trunk_indices[selected_pos]
            print(f"selected_idx: {selected_idx}", flush=True)
            ...
            if selected_idx not in actual_selected_chunks:
                ...
                # Retrieve the cost of the selected trunk
                selected_cost = next(cost for idx, cost in chunk_cost if idx == selected_idx)
                current_cost_utility += selected_cost  # Add the cost to the total
                print(f"Selected trunk: {selected_idx}, Cost: {selected_cost}, Total Cost: {current_cost_utility}", flush=True)
                
        cost_utility = current_cost_utility
        # while True:
        #     for cluster in sorted_cluster_indices:
        #         while cluster:  # As long as the cluster is not empty
        #             idx = cluster.pop(0)  # Get the first element in the cluster
