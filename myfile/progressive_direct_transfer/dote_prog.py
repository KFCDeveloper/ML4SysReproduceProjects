...
if props.so_mode == "progressive": # draw the fig$2, which contain direct transfer
    ...
    # --- prepare subsets
    dataset_waiting = []
    topo_list = ["Abilene-obj2-2-('5', '8')-('6', '7')","Abilene-obj2-dis05-2-('5', '8')-('6', '7')","Abilene-obj2-dis10-2-('5', '8')-('6', '7')","Abilene-obj2-2-('0', '1')-('0', '2')"]
    ...
    concatenated_dataset = ConcatDataset([subset.dataset for subset in subsets])
    # --- end prepare
    ...
    num_interval = 20   # 20 是我规定的，
    test_num_dpoints = total_length # 相当于时间长度（也是 【推理的次数】），我让它直接等于1个数据集的train数据的总长度，相当于 6000；因为我们是假设四个数据集每个取一个quarter拼起来就是总的timeline
    n_epochs_interval = 10  # 相当于 在间隙的时候，适应新的数据集 需要训多少个epoch
    adapt_interval = test_num_dpoints/num_interval    # 会在整个训练过程中adapt num_interval 次，并且每次adapt 数据的数量都会增加的，我最好再设置一个权重
    ...
    with tqdm(test_dl) as tests:
        ...
        for step ,(inputs, targets) in enumerate(tests):
            interv_num_record = int(step / adapt_interval)
            ...
            if step // adapt_interval == 1 and interv_num_record >= int(num_interval *  test_num_dpoints / 4):  
                model.train()
                #TODO: 每次在这里重新初始化数据集；
                ...
                using_subsets = subsets[0:to_subset_index]
                old_datasets = subsets[0:to_subset_index - 1] # 因为interv_num_record >= int(num_interval *  test_num_dpoints / 4)，所以一定是有olddataset的(to_subset_index>)
                ...
                selected_dealing_indices = list(test_num_dpoints* (interv_num_record // (num_interval/4))/num_interval)
                ...
                for dataset in old_datasets:
                    ...
                    if len(dataset) >= part_len:
                        # 随机选择长度为 part_len 的索引
                        indices = torch.randperm(len(dataset))[:part_len / 4]
                        selected_old_subsets.append(Subset(dataset, indices))
                    else:
                        # 如果长度不足，直接使用整个数据集
