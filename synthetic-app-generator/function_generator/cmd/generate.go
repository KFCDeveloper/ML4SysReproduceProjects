...
var (
	allSizes               = []int{128, 256, 512, 1024, 1536, 2048}
	allCPUSizes            = []int{128, 256, 512, 1024, 1536, 2048}
...
func runGenerate(cmd *cobra.Command, args []string) {
	...
	for _, rolled := range allRolls {
		functionNamePrefix := strings.ToLower(strings.Join(rolled, "_"))

		for _, functionSize := range *sizes {
			for _, cpu := range allCPUSizes {
				generatedVariables, packages, filesToInclude, funcName, funcSize := generateVariables(rolled, functionSize, cpu, functionNamePrefix)

				os.MkdirAll(fmt.Sprintf("./build/%s/%s/", functionNamePrefix, funcName), 0777)
				...
				copyFilesToInclude(filesToInclude, funcName, functionNamePrefix)
...
func generateVariables(functionSegments []string, functionSize int, cpu int, functionNamePrefix string) (subgeneratorData, []string, includedFilesData, string, int) {
	var result subgeneratorData
	functionName := fmt.Sprintf("%s_cpu_%d_mem_%d", functionNamePrefix, cpu, functionSize)
...
func copyFilesToInclude(filesToInclude includedFilesData, funcName string, functionNamePrefix string) {
	for file, funcSegment := range filesToInclude {
		err := util.CopyFile(
			filepath.Join(functionSegmentsFolder, funcSegment, file),
			filepath.Join("build", functionNamePrefix, funcName, file))
		if err != nil {
...
func executeTemplate(templateFileName string, variables generatorData) {
	...
	targetFile, err := os.Create(fmt.Sprintf("./build/%s/%s/%s", variables.Prefix, variables.FunctionName, templateFileName))
