...
NN_MODEL = None
# os.environ['CUDA_VISIBLE_DEVICES'] ="0" # ydy: changed
...
def testing(epoch, nn_model, log_file):
    ...
    # run test script
    os.system('/mydata/miniconda3/envs/pensieve/bin/python rl_test.py ' + nn_model)
...
def agent(agent_id, all_cooked_time, all_cooked_bw, net_params_queue, exp_queue):
    ...
    with tf.Session() as sess, open(LOG_FILE + '_agent_' + str(agent_id), 'wb') as log_file: # ydy: change wb to w 
        actor = a3c.ActorNetwork(sess,
                                 state_dim=[S_INFO, S_LEN], action_dim=A_DIM,
                                 learning_rate=ACTOR_LR_RATE)
        critic = a3c.CriticNetwork(sess,
        ...
        while True:  # experience video streaming forever
            ...
            log_file.write(str(time_stamp).encode() + b'\t' +
                           str(VIDEO_BIT_RATE[bit_rate]).encode() + b'\t' +
                           str(buffer_size).encode() + b'\t' +
                           str(rebuf).encode() + b'\t' +
                           str(video_chunk_size).encode() + b'\t' +
                           str(delay).encode() + b'\t' +
                           str(reward).encode() + b'\n')
            log_file.flush()
