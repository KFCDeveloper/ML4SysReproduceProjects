...
if props.so_mode == SOMode.TRAIN: #train
    ...
    initialize = True
    active_indices,pool_indices,initialize = AL_Select(train_dl,initialize,i)
    start_time = time.time()
    cost_utility = 0
    subset_sampler = SubsetRandomSampler(active_indices)
    subset_dataloader = DataLoader(train_dl.dataset, batch_size=batch_size, sampler=subset_sampler)
    #smooth
    ...
    for epoch in range(n_epochs):
        ...
        with tqdm(subset_dataloader) as tepoch:
            epoch_train_loss = []
        ...
        data_uncertainty,data_cost,new_labeled,sample_scale = uncertainties_simulation(train_dl,dataset_size,sample_size,iter_training_cost,pool_indices,model,opt,device)
        ##metrics to stop al
        ...
        if result == True:
            ...
            if i>=6:
                break
            number,new_indices,cost_utility = active_learning_iteration(cost_utility,i,model, train_dl, pool_indices, batch_size, device, opt)
